"""Different coherence (context, key-entity) calculation, and 
    disambiguation.
"""
...
def entity_to_context_scores_armin(candslist, direction, method):
...
def entity_to_context_scores_kaveh(candslist, direction, method):
    
    ''' finds the similarity between each entity with other entities
        Inputs:
            candslist: the list of candidates [[(c11, p11),...(c1k, p1k)],...[(cn1, pn1),...(c1m, p1m)]]
            direction: embedding direction
            method: similarity method
            iteration: iteration number of scores calculation
        Returns:
           cvec_arr: Candidate embeddings, a two dimensional array, each column represent 
           cveclist_bdrs: a list of pairs (beginning, end), to indicate where the 
                   reperesentation of the candidates for cij reside        
           cands_score_list: scroes in the form of [[s11,...s1k],...[sn1,...s1k]] (k=#all candidates of all mentions), or in the form of [[s111,s112,...,s11m1],...,[snn1,...,snnmn]]
                    where sij  is the similarity of c[i,j] to to c[i',j']
                    
            '''
   # print "hello this is vsmcoherence"
    cvec_arr, cveclist_bdrs =  get_candidate_representations(candslist, direction, method)    
    ...
    for a in range(iteration):
        for i in range(len(candslist)):
            ...
            for v in cvec:
                total = 0
                for k in range(len(candslist)):
                    if k !=i:
                        bk,ek = cveclist_bdrs[k]
                        cveck = cvec_arr[bk:ek]
                        counter = 0
                        for l in cveck:
                            total = total + (repco[k][counter])*(1-sp.spatial.distance.cosine(v , l));
                            counter+=counter;
                ...
                s.append(total)
            cands_score_list.append(s)
        repco = updaterepco(cands_score_list)
        return cvec_arr, cveclist_bdrs, cands_score_list



def updaterepco(cands_score_list):
    repco = []
    for i in cands_score_list:
        s = []
        k = sum(i)
        if k !=0:
...
def entity_to_context_scores_kave2(candslist, direction, method):
    ...
    ''' finds the similarity between each entity with other entities
        ...
        Returns:
           ...
           cands_score_list: scroes in the form of [[s11,...s1k],...[sn1,...s1k]] (k=#all candidates of all mentions), or in the form of [[s111,s112,...,s11m1],...,[snn1,...,snnmn]]
            ...
            '''
 #   print "hello this is vsmcoherencae2"
    cvec_arr, cveclist_bdrs =  get_candidate_representations(candslist, direction, method)    
...
def coherence_scores_driver(C, ws=3, method='rvspagerank', direction=DIR_BOTH, op_method="keydisamb"):
