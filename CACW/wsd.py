...
def get_scores(S, M, C, method):
    ...
    if method == 'keydisamb_max':
        scores = coherence_scores_driver(C,ws=6, method='rvspagerank', direction=DIR_BOTH, op_method='keydisamb', overlap_method='max', stride=3)
...
def wsd(S, M, C, method='learned'):
    ...
    print find_max(C,candslist_scores)
